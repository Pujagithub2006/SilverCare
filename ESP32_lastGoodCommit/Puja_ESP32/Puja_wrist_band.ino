...
// =========================================================================
//  SILVERCARE - SENIOR SAFETY WRIST BAND (ESP32 BLE SERVER + CLIENT)
//  MUTUAL PAIRING VERSION
// =========================================================================
...
// =========================================================================
bool mutualPairingConfirmed() {
    return deviceConnected && clientLinkVerified;
}
...
void connectToWaist(NimBLEAddress address) {
    ...
    if (pWaistPairChar != nullptr) {
        ...
        if (pWaistPairChar->canWrite()) {
            String pairMsg = "PAIR:" + uniqueDeviceID;
            pWaistPairChar->writeValue(pairMsg.c_str(), pairMsg.length());
            Serial.println("📤 [BLE-CLIENT] Sent PAIR request to Waist Belt (ID: " + uniqueDeviceID + ")");
        } else {
...
void loop() {
    ...
    if (millis() - lastBLESend > 500) {
        if (mutualPairingConfirmed()) {
            ...
            Serial.println("⏳ Waist Belt connected to us, but mutual pairing not yet confirmed - holding data");
        }
        lastBLESend = millis();
    }
...
void setup() {
    ...
    Serial.println("========================================");
    Serial.println("=== SILVERCARE WRIST BAND (MUTUAL PAIRING) ===");
    Serial.println("========================================");
